...
v282(v295, "StealSpeed", "Travel speed", "Studs/s — capped at 600 for stable delivery", 50, 600, 600, " studs/s")
v263(v295, "Targeting")
...
v282(v298, "BypassCap", "Bypass speed cap", "studs per second while walking with bypass on — max 600", 150, 600, 600, " studs/s")
v263(v298, "Flight")
v281(v298, "Flight", "Flight", "WASD to fly, Space up, Left Ctrl down — holds altitude when you let go", false)
v282(v298, "FlightSpeed", "Flight speed", "studs per second while flying — max 600", 150, 600, 600, " studs/s")
...
(function()
    ...
    local function v1151()
        if t9.Flight then
            return math.clamp(tonumber(t9.FlightSpeed) or 600, 150, 600)
        end

        if u1128 and typeof(u1127) == "Vector3" then
            return math.clamp(tonumber(t9.StealSpeed) or 600, 50, 600)
        end

        if t9.BypassSpeed == true then
            return math.clamp(tonumber(t9.BypassCap) or 600, 150, 600)
        end
    ...
    local function v1182(p218)
        ...
        if v1682 then
            if v1685 then
                pcall(function()
                    if v1686 then
                        v1686.WalkSpeed = math.clamp(v1151(), 16, 600)
                    end
                end)
        ...
        pcall(function()
            ...
            if v1697 then
                ...
                if u1698 then
                    ...
                    v1686.WalkSpeed = math.clamp(v2771, 16, 600)
                end
            elseif t9.BypassSpeed == true then
                v1686.WalkSpeed = math.clamp(tonumber(t9.BypassCap) or 600, 16, 600)
            else
                v1686.WalkSpeed = n9
            end
            ...
            if type(v) == "table" then
                ...
                if v1980 then
                    ...
                    if t9.AutoEvent == true then
                        local eventArea = string.lower(tostring(v.AreaId or ""))
                        local eventName = string.lower(tostring(v1980.name or ""))
                        local isLightDarkBiome = eventArea:find("angel", 1, true) ~= nil
                            or eventArea:find("demon", 1, true) ~= nil
                            or eventArea:find("light", 1, true) ~= nil
                            or eventArea:find("dark", 1, true) ~= nil
                            or eventArea:find("darkness", 1, true) ~= nil
                            or eventName:find("angel", 1, true) ~= nil
                            or eventName:find("demon", 1, true) ~= nil

                        if isLightDarkBiome then
                            local str25 = tostring(v.AreaId or "")
                            local v1987 = v1196(t9.Areas)
                            v1984 = not next(v1987) or v1987[string.lower(str25)] == true
                        end
                    end
    ...
    local function v1224(p326, p327)
        ...
        -- Delivery wall bypass: while carrying, choose a waypoint just outside the
        -- wall/treadmill rectangle and approach the safe zone along its edge.
        -- This avoids pushing directly through the wall and reduces delivery drops.
        if t216 and t216.carrying == true and v2068 then
            local margin = 14
            local cx, cz = v2068.cx, v2068.cz
            local hx, hz = v2068.hx + margin, v2068.hz + margin
            local candidates = {
                Vector3.new(cx - hx, p326Y, cz - hz),
                Vector3.new(cx - hx, p326Y, cz + hz),
                Vector3.new(cx + hx, p326Y, cz - hz),
                Vector3.new(cx + hx, p326Y, cz + hz),
                Vector3.new(cx - hx, p326Y, cz),
                Vector3.new(cx + hx, p326Y, cz),
                Vector3.new(cx, p326Y, cz - hz),
                Vector3.new(cx, p326Y, cz + hz),
            }
            local best, bestScore
            for i = 1, #candidates do
                local c = candidates[i]
                local from = Vector3.new(p326.X, 0, p326.Z)
                local safe = Vector3.new(v2066.X, 0, v2066.Z)
                local wp = Vector3.new(c.X, 0, c.Z)
                local score = (from - wp).Magnitude + (wp - safe).Magnitude
    ...
    function t216.applyCarryHover(root)
        if not root or not root:IsA("BasePart") then
            return
        end

        if not t216.carrying or (t216.state ~= "Return" and t216.state ~= "Bank") then
            t216.carryHoverY = nil
            return
        end
        ...
        local hit = workspace:Raycast(root.Position + Vector3.new(0, 3, 0), Vector3.new(0, -14, 0), params)
        local groundY = hit and hit.Position.Y or (root.Position.Y - 3)
        local desiredY = groundY + 4.5

        if t216.carryHoverY == nil or math.abs(t216.carryHoverY - desiredY) > 2.5 then
            t216.carryHoverY = desiredY
        else
            t216.carryHoverY += math.clamp(desiredY - t216.carryHoverY, -0.35, 0.35)
        end

        local p = root.Position
        local y = math.clamp(t216.carryHoverY, groundY + 3.8, groundY + 5.2)
        local look = root.CFrame.LookVector
        local flatLook = Vector3.new(look.X, 0, look.Z)

        if flatLook.Magnitude > 0.001 then
            root.CFrame = CFrame.lookAt(Vector3.new(p.X, y, p.Z), Vector3.new(p.X, y, p.Z) + flatLook.Unit)
        else
    ...
    function t216.wallUp()
        if t9.BypassSpeed == true or t9.AutoSteal == true and t216 and t216.carrying == true then
            return false
        end
    ...
    local u1230 = (function()
        ...
        local function v2210()
            if t9.AutoEvent == true and t216 and t216.target and t216.target.event then
                return t216.target
            end
    ...
    u1135 = (function()
        ...
        local function v2250()
            ...
            local v4432
            repeat
                if g4410 or ((tonumber(t9.ReadyEarly) or 4) >= v1228() or (t216 and t216.allowTrain and t216.allowTrain() ~= true)) then
                    g4410 = false
                        ...
                        if g4411 or #v4412 > 0 and v4413 < v2231() then
                            ...
                            if not g4410 then
                                ...
                                local ok82, result78, v4430 = pcall(function()
                                    return local wearOk = pcall(v2219, "Treadmill", "AskWearStill")
                                if not wearOk then
                                    task.delay(0.12, function()
                                        pcall(v2219, "Treadmill", "AskWearStill")
                                    end)
                                end
                                end)

                                if not (ok82 and result78 == true) then
                                    task.wait(0.12)
                                    ok82, result78, v4430 = pcall(function()
    ...
    t216._carryHoverConnection = t216._carryHoverConnection or RunService.Heartbeat:Connect(function()
        pcall(function()
            local Character = LocalPlayer.Character
            local Root = Character and Character:FindFirstChild("HumanoidRootPart")
            t216.applyCarryHover(Root)
        end)
    end)
        ...
        if t216.carrying then
            ...
            if t216.state == "Grab" or t216.state == "GoEgg" or t216.state == "Chase" then
                if t216.target and t216.target.event then
                    ...
                    t216.lookWhy = "current event egg"

                    if t216.state ~= "Return" then
                        v1102("steal", t216.state, "->", "Return", "current event egg", t216.target and t216.target.name or "")
                        t216.state = "Return"
                        t216.since = os.clock()
                    else
                        t216.state = "Return"
    ...
    function t216.allowTrain()
        ...
        if t216.carrying == true then
            return false
        end

        if t9.TrainWhenIdle ~= true and not t216.running then
            return false
        end
    ...
    function u1132(p475)
        ...
        if u1127 and not u1116 then
            ...
            pcall(function()
                ...
                if u1128 and typeof(u1127) == "Vector3" then
                    ...
                    v2505.WalkSpeed = math.clamp(math.min(v4575, 600), 16, 600)
                else
                    v2505.WalkSpeed = n9
                end
        ...
        if t216.state == "Return" then
            ...
            if v1227(v2504) then
                if t216.liveCarry and select(1, t216.liveCarry()) then
                    ...
                    t216.deliverySettleAt = t216.deliverySettleAt or (os.clock() + 1.25)
                    v1231()
                    return
                end

                t216.deliverySettleAt = nil
                t216.carrying = false
            ...
            if v2514 > 45 then
                v1102("steal", "Return timeout — retrying edge route")
        ...
        if t216.state == "Bank" then
            ...
            if elapsed46 - t216.lastBankTry >= 0.25 then
                t216.lastBankTry = elapsed46
            end

            local serverCarrying = t216.liveCarry and select(1, t216.liveCarry()) or false

            -- Give the server a stable delivery position inside the safe zone while
            -- the egg is still marked as carried. This is a retry/settle mechanism,
            -- not a client-side success assumption.
            local safePos = select(1, u1219())
            if serverCarrying and typeof(safePos) == "Vector3" and (v2504.Position - safePos).Magnitude > 8 then
                local flat = Vector3.new(safePos.X - v2504.Position.X, 0, safePos.Z - v2504.Position.Z)
                if flat.Magnitude > 0.1 then
                    u1127 = Vector3.new(safePos.X, v2504.Position.Y, safePos.Z)
                end
            end

            if not serverCarrying then
                t216.deliverySettleAt = nil
                t216.carrying = false
                t216.carryHoverY = nil
                v1232("server-confirmed-delivery")
