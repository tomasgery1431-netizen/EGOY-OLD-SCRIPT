-- EGOYHUB Professional Transparent UI Theme
-- Visual-only styling helper; gameplay logic remains unchanged.
...
v282(v295, "StealSpeed", "Travel speed", "Studs/s — capped at 700 for stable egg returns", 50, 700, 700, " studs/s")
v263(v295, "Targeting")
...
v263(v295, "Event")
v281(v295, "AutoEvent", "Auto current event", "uses the current supported event handler", false)
...
v263(v298, "Walking")
v281(v298, "WallBypass", "Wall bypass", "uses the egg-reset wall state only as a route barrier when disabled", true)
...
v282(v298, "BypassCap", "Bypass speed cap", "studs per second while walking with bypass on", 150, 700, 700, " studs/s")
v263(v298, "Flight")
...
(function()
    ...
    local function v1151()
        ...
        if t9.BypassSpeed == true then
            return math.clamp(tonumber(t9.BypassCap) or 700, 150, 700)
        end
    ...
    local function v1182(p218)
        ...
        pcall(function()
            ...
            elseif t9.BypassSpeed == true then
                v1686.WalkSpeed = math.clamp(tonumber(t9.BypassCap) or 700, 16, 700)
            else
                v1686.WalkSpeed = n9
            end
                        ...
                        if g4411 or #v4412 > 0 and v4413 < v2231() then
                            ...
                            if not g4410 then
                                ...
                                if not ok82 or result78 ~= true then
                                    task.wait(0.12)
                                    pcall(function()
                                        ok82, result78, v4430 = true, v2219("Treadmill", "AskWearStill")
                                    end)
                                end
                                local v4431 = t291
    ...
    function t216.applyCarryHover(root)
        if not root or not root:IsA("BasePart") or not t216.carrying then
            return
        end
        if t216.state ~= "Return" and t216.state ~= "Bank" then
            return
        end
        local floor = t216.floorY()
        if type(floor) ~= "number" then
            return
        end
        local y = floor + 4
        if root.Position.Y < y - 2 or root.Position.Y > y + 8 then
            pcall(function()
        ...
        if u1127 and not u1116 then
            ...
            pcall(function()
                ...
                if u1128 and typeof(u1127) == "Vector3" then
                    ...
                    v2505.WalkSpeed = math.clamp(v4575, 16, 700)
                else
                    v2505.WalkSpeed = n9
                end
